...
const ADMIN_NAV = [
  ...
  { to: "assign",     label: "Assign Streams",  icon: Settings        },
...
// Marketing nav — events, feedback only
const MARKETING_NAV = [
  { to: "events",   label: "Events",   icon: Ticket         },
  { to: "feedback", label: "Feedback", icon: MessageSquare  },
];
...
export default function AdminLayout() {
  const { user, signOut, isMarketing } = useAuth();
  const navigate = useNavigate();

  const NAV = isMarketing ? MARKETING_NAV : ADMIN_NAV;
  const roleLabel = isMarketing ? "Marketing" : "Admin";
    ...
    <div className="min-h-screen bg-[#F7F4ED] flex" style={{ fontFamily: "ui-sans-serif,system-ui,sans-serif" }}>
      ...
      <div className="w-60 flex-shrink-0 bg-[#1A2830] flex flex-col">
        ...
        <div className="p-5 border-b border-white/10">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-white/40 uppercase tracking-widest">{roleLabel}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] bg-brand-gold/20 text-brand-gold px-2 py-0.5 rounded ml-auto uppercase tracking-widest">
              {roleLabel}
            </span>
          </div>
        </div>
        ...
        <nav className="flex-1 p-3 overflow-y-auto">
          <p className="text-[10px] text-white/30 uppercase tracking-widest px-3 mb-3 mt-2">
            {isMarketing ? "Marketing" : "Operations"}
          </p>
          ...
          {/* Marketing sees partner calendar */}
          {isMarketing && (
            <button onClick={() => navigate("/partner/calendar")}
              ...
              <LayoutDashboard size={16}/> Partner Calendar
            </button>
          )}

          {/* Admin sees partner ops */}
          {!isMarketing && (
