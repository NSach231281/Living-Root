...
import {
  Plus, Edit2, Trash2, CheckCircle, X, Eye, EyeOff,
...
type Tab = "events" | "bookings" | "registrations";
...
export default function AdminEvents() {
  ...
  const save = async () => {
    ...
    const data = { ...form, seatsLeft: editId ? form.seatsLeft : form.seatsTotal, createdBy: "admin" };
  ...
  const confirmBooking = (id: string) => update(ref(db, `/bookings/${id}`), { status:"confirmed", paymentStatus:"paid", confirmedAt:Date.now() });
  ...
  const filteredRegs = regFilter === "all"
    ? registrations
    : registrations.filter(r => r.event_id === regFilter);

  const regEventOptions = Array.from(new Set(registrations.map(r => r.event_id)))
    .map(eid => ({ eid, title: registrations.find(r => r.event_id === eid)?.event_title || eid }));

  const exportCSV = () => {
    const rows = [
      ["Ticket Ref","Event","Name","Email","Phone","Seats","Status","Registered At"],
      ...filteredRegs.map(r => [
        r.ticket_ref, r.event_title, r.name, r.email, r.phone,
        r.seats, r.status, new Date(r.created_at).toLocaleString("en-IN"),
      ]),
    ];
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-brand-muted">Create events · manage bookings · view registrations</p>
        </div>
      ...
      <div className="flex gap-0 border-b border-brand-border mb-6">
        {([["events","Events"],["bookings","Bookings"],["registrations","Registrations"]] as [Tab,string][]).map(([k,l]) => (
          <button key={k} onClick={() => setTab(k)}
            ...
            {l}
            {k==="bookings"&&pending.length>0&&<span className="ml-1 px-2 py-0.5 bg-orange-100 text-orange-600 rounded-full text-xs">{pending.length}</span>}
            {k==="registrations"&&<span className="ml-1 px-2 py-0.5 bg-brand-sand text-brand-clay rounded-full text-xs">{registrations.length}</span>}
          </button>
        ))}
      </div>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-brand-border p-6 mb-6 shadow-card">
          ...
          <div className="grid md:grid-cols-2 gap-5">
            <div className="md:col-span-2"><label className="label">Event title *</label><input value={form.title} onChange={e=>setForm((p: any)=>({...p,title:e.target.value}))} className="input" placeholder="e.g. Friday Jam Night"/></div>
            <div className="md:col-span-2"><label className="label">Description</label><textarea value={form.description} onChange={e=>setForm((p: any)=>({...p,description:e.target.value}))} className="input" rows={3}/></div>
            ...
            <div><label className="label">Original Price (₹) — optional</label><input type="number" value={form.originalPrice||""} min={0} onChange={e=>setForm((p: any)=>({...p,originalPrice:Number(e.target.value)}))} className="input"/>
              {form.originalPrice > form.price && form.price > 0 && (<p className="text-xs text-emerald-600 font-bold mt-1">{Math.round(((form.originalPrice - form.price) / form.originalPrice) * 100)}% OFF</p>)}
            </div>
            ...
            <div><label className="label">Revenue stream</label><select value={form.stream} onChange={e=>setForm((p: any)=>({...p,stream:e.target.value}))} className="input">{STREAMS.map(s=><option key={s} value={s}>{s}</option>)}</select></div>
              ...
              <div className="flex flex-wrap gap-2">
                {VIBES.map(v=>(
                  <button key={v} type="button" onClick={()=>setForm((p: any)=>({...p,vibes:p.vibes.includes(v)?p.vibes.filter((x: string)=>x!==v):[...p.vibes,v]}))}
                    className={`px-3 py-1.5 text-xs font-bold uppercase tracking-widest rounded-full border transition-all ${form.vibes.includes(v)?"bg-brand-earth text-white border-brand-earth":"border-brand-border text-brand-clay hover:border-brand-spice"}`}>{v}</button>
                ))}
              </div>
            </div>
            <div><label className="label">Status</label><select value={form.status} onChange={e=>setForm((p: any)=>({...p,status:e.target.value}))} className="input"><option value="draft">Draft</option><option value="live">Live</option><option value="sold_out">Sold out</option><option value="completed">Completed</option></select></div>
          </div>
      ...
      {tab === "events" && (
        <div className="flex flex-col gap-3">
          {eventList.length === 0 ? (
            ...
            return (
              <div key={ev.id}>
                <div className="bg-white rounded-2xl border border-brand-border p-5 flex flex-wrap items-center gap-5 shadow-card">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-brand-clay">{ev.date} · {ev.time} · {ev.price === 0 ? "Free" : `₹${ev.price}`}</p>
                    <p className="text-xs text-brand-muted mt-1">{booked}/{ev.seatsTotal} booked · {evRegs.length} public registrations</p>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button onClick={() => setShareEventId(shareEventId === ev.id ? null : ev.id)}
                      className="p-2 rounded-lg border border-brand-border hover:border-brand-spice text-brand-muted hover:text-brand-spice transition-colors" title="Share / QR">
                      <Share2 size={16}/>
                    </button>
                    <button onClick={()=>toggleStatus(ev.id,ev)} className="p-2 rounded-lg border border-brand-border hover:border-brand-spice text-brand-muted hover:text-brand-spice transition-colors">
                ...
                {shareEventId === ev.id && (
                  <div className="border-x border-b border-brand-border rounded-b-2xl -mt-2 pt-2">
                    <SharePanel
                      eventId={ev.id}
                      eventTitle={ev.title}
                      eventDate={ev.date}
        ...
        <div>
          {pending.length > 0 && (
            <div className="mb-6">
              <h2 className="font-serif text-lg font-bold text-brand-earth mb-3 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-orange-400 animate-pulse inline-block"/> Pending ({pending.length})
              </h2>
              <div className="flex flex-col gap-3">
                {pending.map(b => (
                  <div key={b.id} className="bg-orange-50 rounded-2xl border border-orange-200 p-5 flex flex-wrap items-center gap-5">
                    ...
                    <div className="flex gap-2">
                      <button onClick={()=>confirmBooking(b.id)} className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white text-sm font-bold rounded-xl hover:bg-green-700 transition-colors">
          ...
          <div className="bg-white rounded-2xl border border-brand-border overflow-hidden shadow-card">
            <table className="w-full text-sm">
              <thead><tr className="bg-brand-sand">{["Event","Customer","Seats","Amount","Payment","Status","Date"].map(h=><th key={h} className="px-4 py-3 text-left text-xs font-bold uppercase tracking-widest text-brand-muted">{h}</th>)}</tr></thead>
              <tbody>
                {bookingList.map(b=>(
      ...
      {tab === "registrations" && (
        <div>
          <div className="flex flex-wrap items-center gap-3 mb-5">
            <div className="flex-1">
              <h2 className="font-serif text-lg font-bold text-brand-earth">Public Registrations</h2>
              <p className="text-xs text-brand-muted">From the public /register/:id form — no login required</p>
            </div>
            <select
              value={regFilter}
            ...
            >
              ...
              {regEventOptions.map(({ eid, title }) => (
                <option key={eid} value={eid}>{title}</option>
              ))}
            </select>
            <button onClick={exportCSV} className="btn-outline text-xs px-3 py-2 flex items-center gap-1.5">
          ...
          {regLoading ? (
            ...
            <div className="text-center py-16 bg-white rounded-2xl border border-brand-border text-brand-muted">
              <p className="text-3xl mb-3">📋</p>
              <p className="font-serif text-lg text-brand-earth mb-1">No registrations yet</p>
              <p className="text-sm">Share an event's QR code or registration link to start collecting registrations.</p>
            </div>
          ) : (
            <div className="bg-white rounded-2xl border border-brand-border overflow-x-auto shadow-card">
              <table className="w-full text-sm min-w-[700px]">
                <thead>
                  <tr className="bg-brand-sand">
                    {["Ticket Ref","Event","Name","Email","Phone","Seats","Status","Email ✓","WA ✓","Registered"].map(h=>(
                ...
                <tbody>
                  {filteredRegs.map(r => (
                    <tr key={r.id} className="border-t border-brand-border hover:bg-brand-sand/30 transition-colors">
                      <td className="px-4 py-3 font-mono text-xs font-bold text-brand-spice">{r.ticket_ref}</td>
                      <td className="px-4 py-3 font-medium text-brand-earth max-w-[160px] truncate">{r.event_title}</td>
                      <td className="px-4 py-3 text-brand-clay">{r.name}</td>
                      <td className="px-4 py-3 text-brand-clay text-xs">{r.email}</td>
                      <td className="px-4 py-3 text-brand-clay text-xs">{r.phone}</td>
                      <td className="px-4 py-3 text-center font-bold">{r.seats}</td>
                      <td className="px-4 py-3">
                        <span className={`badge text-xs ${r.status==="confirmed"?"bg-green-100 text-green-700":r.status==="pending_payment"?"bg-amber-100 text-amber-700":"bg-red-100 text-red-600"}`}>
                          {r.status === "pending_payment" ? "⏳ pending" : r.status}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-center">{r.email_sent ? "✅" : "–"}</td>
                      <td className="px-4 py-3 text-center">{r.whatsapp_sent ? "✅" : "–"}</td>
